...
public class Main {
    ...
    static void tambahReservasi() {
        ...
        if (tanggal.trim().isEmpty()) {
            reservasi = new Reservation(customer, service);
        } else {
            reservasi = new Reservation(customer, service, tanggal);
        }
    ...
    static void pembayaran() {
        ...
        if (daftarReservasi.isEmpty()) {
            System.out.println("(╥﹏╥) Belum ada reservasi.");
            return;
        }

        lihatReservasi();

        System.out.print("Pilih nomor reservasi untuk bayar: ");
        int index = input.nextInt() - 1;
        input.nextLine();
